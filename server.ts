...
async function startServer() {
  ...
  app.get("/api/tickets", async (req, res) => {
    ...
    function isGamePlayed(dateStr: string, timeStr?: string): boolean {
    ...
    const fallbackGames = [
      ...
      },
      {
        ...
        date: "2026-09-14",
        time: "17:15",
        competition: "Monday Night Football",
        status: "Upcoming · 5:15 PM",
        cheapestPrice: 110,
        vipPrice: 750,
      ...
      },
      {
        ...
        date: "2026-09-20",
        time: "13:00",
        competition: "NFL Week 2 · Historic Rivalry",
        status: "Upcoming · 1:00 PM",
        cheapestPrice: 95,
        vipPrice: 650,
      ...
      },
      {
        ...
        date: "2026-09-20",
        time: "13:00",
        competition: "NFL Week 2 · AFC East",
        status: "Upcoming · 1:00 PM",
        cheapestPrice: 85,
        vipPrice: 580,
      ...
      },
      {
        ...
        date: "2026-09-24",
        time: "20:15",
        competition: "Thursday Night Football",
        status: "Upcoming · 8:15 PM",
        cheapestPrice: 75,
        vipPrice: 500,
    ...
    const generateFallback = () => {
      // Remove any game that has already been played
      const unplayedGames = fallbackGames.filter(game => !isGamePlayed(game.date, game.time));

      const activeTickets = unplayedGames.map(game => {
    ...
    try {
      ...
      let filtered = formattedEvents.filter((g: any) => !isGamePlayed(g.date, g.time));
      if (teamFilter) {
  ...
  if (!isProduction) {
    ...
    try {
      ...
      app.get("*", (req, res) => res.status(500).send("Development server initialization error."));
    }
  } else {
    ...
    app.get("*", (req, res) => {
      ...
      if (fs.existsSync(indexPath)) {
        ...
        if (fs.existsSync(rootIndex)) {
          ...
          res.status(503).send("Application is starting up, please refresh momentarily.");
        }
      }
    });
  }
